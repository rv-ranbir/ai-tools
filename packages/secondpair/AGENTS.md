...
| `src/config.ts` | `ReviewConfig` Zod schema + `loadConfig()`/`mergeConfig()` (`.pr-review.yml`) + `isIgnored()` + `applyCliOverrides()` (the single place CLI flags — `--fail-on`, `--write-suppressions` — merge onto a loaded config; immutable, never mutates its input). |
