...
/** Run the repomind MCP server on stdio, serving the index in `cwd`. */
export async function runMcpServer(cwd: string): Promise<void> {
  const server = new Server(
    { name: "repomind", version: "0.1.0" },
    { capabilities: { tools: {} } },
  );
  ...
  const requireIndex = async (): Promise<CodemapIndex> => {
    ...
    if (!index) {
      throw new Error(`No repomind index at ${indexPath(cwd)}. Run \`repomind index\` first.`);
    }
    return index;
  };
